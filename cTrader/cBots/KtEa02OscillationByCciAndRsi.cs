...
namespace cAlgo.Robots
{
    ...
    public class KtEa02OscillationByCciAndRsi : Robot
    {
        //CCI指标期数
        ...
        public double stop_loss_pips { get; set; }
        ...
        //rsi数据路径
        //private string path_rsi = "C:\\csharp\\m5_rsi.txt";
        //m5当前值
        private double curr_m5_val;
        //rsi当前值
        private double curr_rsi_val;
        //下单时m5的值
        private double m5_val_when_order = 0;
        //下单时rsi的值
        private double rsi_val_when_order = 0;
        //下单前m5的最低值
        private double m5_min_val_before_order = 0;
        //下单前m5的最高值
        private double m5_max_val_before_order = 0;
        //下单时m5的最低值
        private double m5_min_val_when_order = 0;
        //下单时m5的最高值
        private double m5_max_val_when_order = 0;
        //m5最低值时RSI的值
        private double rsi_val_when_m5_is_min = 50;
        //m5最高值时RSI的值
        private double rsi_val_when_m5_is_max = 50;
        //下单时的价格
        ...
        //日志路径
        private string path_log = "C:\\csharp\\log_65.txt";
        //日志字符串
        ...
        private void Check()
        {
            ...
            string[] data_m5 = GetIndicatorsLatestData("cci", 1000);

            curr_m5_val = GetCurrM5Val(data_m5);



            //string[] data_rsi = getLatestData(path_rsi, 1000);
            ...
            if (curr_m5_val < 0)
            {
                if (is_first_price_change)
                {
                    ...
                    rsi_val_when_m5_is_min = curr_rsi_val;
                }
                else if (curr_m5_val < m5_min_val_before_order)
                {
                    ...
                    m5_min_val_before_order = curr_m5_val;
                    rsi_val_when_m5_is_min = curr_rsi_val;
                    //最小值变更之后，将对应的价格数组清空，重新开始保存值
                    m5_min_to_now_price_al = new ArrayList();
                }
            }
            else
            {
                if (is_first_price_change)
                {
                    ...
                    rsi_val_when_m5_is_max = curr_rsi_val;
                }
                else if (curr_m5_val > m5_max_val_before_order)
                {
                    //如果不是刚启动EA，且当前值比最高值还大，则将最高值更新为当前值
                    m5_max_val_before_order = curr_m5_val;
                    rsi_val_when_m5_is_max = curr_rsi_val;
                    //最大值变更之后，将对应的价格数组清空，重新开始保存值
                    m5_max_to_now_price_al = new ArrayList();
                }
            }
            ...
            if (position != null)
            {
                ...
                //如果没有达到利润点平仓
                if (last_order_type == 2 && curr_m5_val <= -100 && (curr_m5_val <= m5_min_val_when_order - 50 || curr_rsi_val <= rsi_val_when_order - 3) && Symbol.Ask < price_when_order)
                {
                    log_str = "当前m5值<=最低-50或当前rsi<=下单时-3，且价格低于Buy单入场价，平仓（curr_m5_val:" + curr_m5_val.ToString() + ",m5_min_val_when_order:" + m5_min_val_when_order.ToString() + ",curr_rsi_val:" + curr_rsi_val.ToString() + ",rsi_val_when_order:" + rsi_val_when_order.ToString() + ",Symbol.Ask:" + Symbol.Ask.ToString() + ",price_when_order:" + price_when_order.ToString() + "）";
                    Print(log_str);
                    WriteTextToFile(log_str);
                    ClosePosition(position);
                    return;
                }
                if (last_order_type == 1 && curr_m5_val >= 100 && (curr_m5_val >= m5_max_val_when_order + 50 || curr_rsi_val >= rsi_val_when_order + 3) && Symbol.Bid > price_when_order)
                {
                    log_str = "当前m5值>=最高+50或当前rsi>=下单时+3，且价格高于Sell单入场价，平仓（curr_m5_val:" + curr_m5_val.ToString() + ",m5_max_val_when_order:" + m5_max_val_when_order.ToString() + ",curr_rsi_val:" + curr_rsi_val.ToString() + ",rsi_val_when_order:" + rsi_val_when_order.ToString() + ",Symbol.Bid:" + Symbol.Bid.ToString() + ",price_when_order:" + price_when_order.ToString() + "）";
                    Print(log_str);
                    WriteTextToFile(log_str);
                    ClosePosition(position);
                    return;
                ...
                IsFaDongCeLue(data_m5, "如果没有达到利润点平仓,判断是否有下单机会,");
            ...
            log_str = "curr_rsi_val:" + curr_rsi_val.ToString() + ",rsi_val_when_m5_is_min:" + rsi_val_when_m5_is_min.ToString() + ",rsi_val_when_m5_is_max:" + rsi_val_when_m5_is_max.ToString();
            Print(log_str);
            WriteTextToFile(log_str);
        ...
        {
            string[] void_string_arr = new string[] 
            {
                            };
            try
            {
        ...
        private void PlaceOrder(TradeType tt)
        {
            ...
            if (result.IsSuccessful)
            {
                ...
                log_str = "下单时，curr_rsi_val:" + curr_rsi_val.ToString() + ",rsi_val_when_m5_is_min:" + rsi_val_when_m5_is_min.ToString() + ",rsi_val_when_m5_is_max:" + rsi_val_when_m5_is_max.ToString();
                Print(log_str);
                WriteTextToFile(log_str);
                ...
                if (last_order_type != order_type && last_order_type != 0)
                {
                    ...
                    {
                        m5_max_val_before_order = 0;
                        rsi_val_when_m5_is_max = 50;
                    }
                    else if (last_order_type == 2)
                    {
                        m5_min_val_before_order = 0;
                        rsi_val_when_m5_is_min = 50;
                    }
                }

                if (order_type == 1)
                {
                    m5_max_val_when_order = m5_max_val_before_order;
                }
                else
                {
                    m5_min_val_when_order = m5_min_val_before_order;
                }
        ...
        private bool IsQiDong(string[] data, int difference, string note = "")
        {
            if (data.Length < 2)
            {
                return false;
            ...
            for (var i = data.Length - 2; i >= 0; i--)
            {
                ...
                if (curr_m5_val <= 0 && curr_m5_val - tmp_val >= difference)
                {
                    //-100以下，呈上行趋势，启动Buy策略
                    //m5符合下单条件，再判断RSI是否符合，若不符合，可能是价格背离的情况，则不下单

                    if (CheckRsiIsMatch(2) == false)
                    {
                        log_str = note + "CCI指标达到下Buy单条件，但是RSI指标未达到，可能价格背离";
                        Print(log_str);
                        WriteTextToFile(log_str);
                        return false;
                    }
                ...
                if (curr_m5_val <= 0 && tmp_val - curr_m5_val >= difference)
                {
                    //-100以下，呈下行趋势，不操作
                    return false;
                }
                if (curr_m5_val >= 0 && tmp_val - curr_m5_val >= difference)
                {
                    ...
                    if (CheckRsiIsMatch(1) == false)
                    {
                        log_str = note + "CCI指标达到下Sell单条件，但是RSI指标未达到，可能价格背离";
                        Print(log_str);
                        WriteTextToFile(log_str);
                        return false;
                    }
        ...
        //判断是否应该发动策略
        private void IsFaDongCeLue(string[] data_m5, string note = "")
        {
            if (curr_m5_val <= -300)
            {
                qidong = IsQiDong(data_m5, 150, note);
                if (qidong == true)
                {
                    order_type = 2;
            ...
            else if (curr_m5_val > -300 && curr_m5_val <= -200)
            {
                qidong = IsQiDong(data_m5, 120, note);
                if (qidong == true)
                {
                    order_type = 2;
            ...
            else if (curr_m5_val > -200 && curr_m5_val <= -150)
            {
                qidong = IsQiDong(data_m5, 80, note);
                if (qidong == true)
                {
                    order_type = 2;
            ...
            else if (curr_m5_val > -150 && curr_m5_val <= -100)
            {
                qidong = IsQiDong(data_m5, 50, note);
                if (qidong == true)
                {
                    order_type = 2;
            ...
            else if (curr_m5_val > 100 && curr_m5_val <= 150)
            {
                qidong = IsQiDong(data_m5, 50, note);
                if (qidong == true)
                {
            ...
            else if (curr_m5_val > 150 && curr_m5_val <= 200)
            {
                qidong = IsQiDong(data_m5, 80, note);
                if (qidong == true)
                {
            ...
            else if (curr_m5_val > 200 && curr_m5_val <= 300)
            {
                qidong = IsQiDong(data_m5, 120, note);
                if (qidong == true)
                {
        ...
        private double PriceQuShi(int m5_qu_shi)
        {
            ...
            if (m5_qu_shi == 1)
            {
                return (double)num_less / num_total;
            }
            else
            {
                return (double)num_greater / num_total;
            }

        }

        //检测RSI是否符合
        private bool CheckRsiIsMatch(int order_type)
        {
            if (order_type == 1)
            {
                //Sell
                if (curr_rsi_val < rsi_val_when_m5_is_max)
                {
                    return true;
                }
                return false;
            ...
            {
                //Buy
                if (curr_rsi_val > rsi_val_when_m5_is_min)
                {
                    return true;
                }
                return false;
